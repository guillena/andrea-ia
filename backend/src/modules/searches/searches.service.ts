...
export class SearchesService {
  ...
  async findOne(id: string, companyId: string) {
    ...
    if (!c) throw new NotFoundException('Búsqueda no encontrada');
    return c;
  }

  async create(companyId: string, userId: string, dto: CreateSearchDto) {
  ...
  async update(id: string, companyId: string, dto: UpdateSearchDto) {
  ...
  async getStats(id: string, companyId: string) {
    ...
    const scores = await this.prisma.score.findMany({
      where: { session: { candidate: { campaignId: id } } },
